...
/// A builder for creating an [`SIntVec`] from a slice of `i64`.
///
/// This builder handles the conversion from signed to unsigned integers by applying
/// the ZigZag transformation. The resulting stream of `u64` values is then
/// passed to the underlying [`IntVec`] builder for compression.
///
/// # Limitations
///
/// This builder does not support automatic parameter selection for codecs, as the
/// data is transformed and consumed as a stream. You must provide a `VariableCodecSpec`
