...
fn main() {
    ...
    data.mem_dbg(DbgFlags::default()| DbgFlags::HUMANIZE | DbgFlags::PERCENTAGE);

    // Create an IntVec with a generic Delta encoding.
    let delta_intvec = LEIntVec::builder(&data)
        .codec(VariableCodecSpec::Delta)
        .build()
        .unwrap();

    println!("\nSize of the IntVec with delta encoding:");
    delta_intvec.mem_dbg(DbgFlags::default()| DbgFlags::HUMANIZE | DbgFlags::PERCENTAGE);
    ...
    auto_intvec.mem_dbg(DbgFlags::default()| DbgFlags::HUMANIZE | DbgFlags::PERCENTAGE);
